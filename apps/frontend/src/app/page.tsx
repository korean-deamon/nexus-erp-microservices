  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#020203] text-zinc-200 font-sans relative overflow-hidden">
      ...
      <main className="flex-1 p-4 md:p-8 pb-32 lg:pb-8 overflow-y-auto custom-scrollbar">
        ...
        {activeTab === 'admin' && (
            <div className="mt-12 mb-6 border-b border-white/5 pb-4">
               <h2 className="text-2xl font-black text-white uppercase tracking-tighter flex items-center gap-3">
                  <ShoppingBag className="w-6 h-6 text-indigo-500" /> Incoming Orders
               </h2>
            </div>
        )}
        ...
        {(activeTab === 'orders' || activeTab === 'admin') && (
           <div className="space-y-6 animate-in fade-in duration-500">
              {Object.entries(groupOrders(isAdmin ? orders : myOrders)).map(([date, items]: any) => (
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-auto border-t sm:border-none pt-3 sm:pt-0">
                             ...
                             <div className="flex gap-2">
                                {isAdmin && activeTab === 'admin' && (
                                   <>
                                      {o.status === 'PENDING' && <button onClick={() => axios.patch(`/api/v1/orders/${o.id}/status`, { status: 'SHIPPED' }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }).then(refreshData)} className="p-2.5 bg-indigo-600 text-white rounded-lg"><Truck className="w-4 h-4" /></button>}
                                      {o.status === 'SHIPPED' && <button onClick={() => axios.patch(`/api/v1/orders/${o.id}/status`, { status: 'DELIVERED' }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }).then(refreshData)} className="p-2.5 bg-sky-600 text-white rounded-lg"><PackageCheck className="w-4 h-4" /></button>}
                                      {o.status === 'DELIVERED' && <button onClick={() => axios.patch(`/api/v1/orders/${o.id}/status`, { status: 'COMPLETED' }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }).then(refreshData)} className="p-2.5 bg-emerald-600 text-white rounded-lg"><CheckCircle className="w-4 h-4" /></button>}
                                   </>
                                )}
